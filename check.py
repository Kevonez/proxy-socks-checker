...
def proxycheckerinit():
	global out_file
	candidate_proxies = open("socks.txt").readlines()
	...
	out_file.close()
	print("\n\nCurrent IPs in proxylist: %s\n" % (len(open("socks.txt").readlines())))

def proxychecker(i):
	proxy = 'http://' + i
	proxy_support = urllib.request.ProxyHandler({'socks' : proxy})
...
def main():
	try:
		...
		print("\n")
		while True:
			choice = input("\nDo you want to check the socks? [Y/n] > ")
